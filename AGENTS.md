...
## ワークフロー
- **ExecPlan ワークフロー**  
  - **目的**: 複雑または長時間かかる開発タスクを成功させるための自己完結型設計書を用意し、誰でも再現できる
    作業手順を提供する。  
  - **トリガー**: 調査を含め数時間以上の作業、広範囲なリファクタリング、新規機能の設計・実装に着手する際。  
  - **手順概要**:  
    1. `docs/meta_plan/exec_plan_guidelines.md` を読み、テンプレートに沿って ExecPlan を作成する。  
    2. 計画に目的・作業ステップ・使用コマンド・検証方法・意思決定ログを記載する。  
    3. 実装中は常に ExecPlan を更新し、進捗と判断を記録する。  
    4. 中断・引き継ぎ時は ExecPlan と最新リポジトリのみで再開できることを確認する。  
  - **参照ドキュメント**: `docs/meta_plan/exec_plan_guidelines.md`
- **ワークフロー追加ワークフロー**  
  - **目的**: 外部資料を基にした新しい運用ルールや開発フローを、このリポジトリの標準形式で共有する。  
  - **トリガー**: ユーザーから URL 等で追加ワークフローの資料が指定された場合。  
  - **手順概要**:  
    1. 既存のワークフローと `docs/directory_structure.md` を確認し、重複がないか整理する。  
    2. `docs/meta_plan/workflow_addition_guidelines.md` に従ってメタドキュメントを作成し、要約と手順を記録する。  
    3. `AGENTS.md` のワークフロー節へ目的・トリガー・参照先を 3〜4 行で追記する。  
  - **参照ドキュメント**: `docs/meta_plan/workflow_addition_guidelines.md`
