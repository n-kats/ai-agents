...
- [x] メッセージ未読キューのデータベース常時永続化とバックエンド切り替えを実装（`docs/exec_plan_message_database.md` 参照）。
...
- [x] LLM 呼び出しを非同期化し、停止イベント受信時に進行中リクエストをキャンセルできる構成を整備。
  ...
  - [x] `shutdown_grace_period` を導入し、応答しない LLM 呼び出しでも終了処理がブロックされない仕組みを追加。
  ...
  - [ ] 詳細計画は `docs/features/async_llm_shutdown.md` を参照（継続タスク）。
...
- [x] サンプル実装ガイドラインを `docs/samples_guideline.md` に集約。
