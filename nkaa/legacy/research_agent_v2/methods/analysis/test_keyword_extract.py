...
class TestKeywordExtractMethod(unittest.TestCase):
    ...
    def setUp(self, mock_load_prompt):
        ...
        self.method = KeywordExtractMethod(llm_client=self.mock_llm_client, settings=self.settings)
    ...
    def test_keyword_extract_method_analyze_success(self):
        ...
        with patch.object(self.method.llm_client, "invoke", return_value=raw_llm_output) as mock_invoke:
            ...
            self.assertEqual(keywords, expected_cleaned_keywords)  # strip されパースされた結果と比較
    ...
    def test_keyword_extract_method_analyze_llm_error(self):
        ...
        with patch.object(self.method.llm_client, "invoke", side_effect=Exception(error_message)) as mock_invoke:
            result = self.method.execute(test_content)

            # アサーション
            mock_invoke.assert_called_once()  # LLMが呼ばれたことを確認
            self.assertIsInstance(result, str)
            self.assertTrue(result.startswith("エラー: キーワード抽出処理中にエラーが発生しました"))
    ...
    def test_keyword_extract_method_analyze_parse_error(self):
        ...
        with patch.object(self.method.llm_client, "invoke", return_value=invalid_llm_output) as mock_llm_invoke:
            result = self.method.execute(test_content)
