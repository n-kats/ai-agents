...
class KeywordExtractMethod(BaseAnalysisMethod):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            logger.error(
                f"キーワード抽出プロンプト '{prompt_name}.j2' の読み込みに失敗: {e}。デフォルトを使用します。"
            )
        ...
        try:
            ...
            logger.error(
                f"キーワード抽出チェーンの初期化に失敗しました: {e}", exc_info=True
            )
            self.chain = None
    ...
    def execute(self, data: Any, **kwargs: Any) -> Any:
        ...
        if not self.chain:
            logger.error(
                f"メソッド '{self.method_name}': キーワード抽出チェーンが初期化されていません。"
            )
        ...
        try:
            # チェーンを実行
            keywords = self.chain.invoke(
                {"text": data, "num_keywords": self.num_keywords}
            )

            if not isinstance(keywords, list):
                logger.warning(
                    f"キーワード抽出結果がリストではありませんでした: {keywords}。空リストを返します。"
                )
                return []
            ...
            logger.info(
                f"メソッド '{self.method_name}' が完了しました。抽出キーワード数: {len(cleaned_keywords)}"
            )
