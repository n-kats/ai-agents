...
class StructuredError(BaseModel):
    ...
    error_code: str = Field(
        ..., description="エラーの種類を示すコード (例: APIError, ValidationError)"
    )
    message: str = Field(..., description="エラーメッセージ")
    details: Optional[Dict[str, Any]] = Field(
        None, description="エラーに関する追加詳細情報"
    )
...
class AgentState(TypedDict):
    ...
    search_results: List[
        Dict[str, Any]
    ]  # 検索結果のリスト。各要素はソース(web/local)、URL/パス、コンテンツを含む辞書
    analysis_results: Dict[
        str, Any
    ]  # 分析メソッドごとの結果を格納する辞書 (例: {"summary": "...", "keywords": [...]})
